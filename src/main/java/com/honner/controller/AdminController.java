...
import com.honner.po.CheckUserId;
...
import com.honner.po.Teacher;
import com.honner.po.TeacherCustom;
import com.honner.po.Userlogin;
...
import com.honner.service.TeacherService;
import com.honner.service.UserloginService;
...
public class AdminController {
    ...
    // 给与学生处分
    @RequestMapping(value = "/chufenStudent", method = {RequestMethod.GET})
    @ResponseBody
    private Map<String, Object> chufen(@RequestParam Integer userId, @RequestParam String content) throws Exception {
        ...
        disposalService.chufen(userId, userlogin.getUserid(), content);
        result.put("msg", "处分成功");
        return result;
    }
    ...
    // 教师页面显示
    @RequestMapping("/shensuList")
    public ModelAndView shensuList(@RequestParam(value = "page", required = true, defaultValue = "1") Integer page,
                                   @RequestParam(value = "pageSize", required = true, defaultValue = "4") Integer pageSize) throws Exception {
        String adminId = (String) SecurityUtils.getSubject().getPrincipal();
        List<Disposal> list = null;

        list = disposalService.findDisposalByAmindIdPageing(Integer.valueOf(adminId), page, pageSize);
        PageInfo disposalInfo = new PageInfo(list);

        ModelAndView mv = new ModelAndView();
        mv.addObject("disposalInfo", disposalInfo);
        mv.setViewName("admin/disposal");
    ...
    // 教师页面显示
    @RequestMapping("/gonggappush")
    public ModelAndView shensuList() throws Exception {
        ModelAndView mv = new ModelAndView();
        mv.setViewName("admin/gonggaopush");
    ...
    public String gonggao() {
        return "admin/gonggaopush";
    }

    @RequestMapping("/gonggaoPush")
    public String gonggaopush(@RequestParam String gonggaopush) {
        adService.updateById(1, gonggaopush);
        return "redirect:/admin/showStudent";
    }
}
